...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        dos = self.data[idx, :-1].unsqueeze(0)
        label = self.data[idx,-1].to(torch.long)
        return dos, label
...
def compute_acc_and_loss(model, train_loader, loss_fn, device):
    ...
    for features, targets in train_loader:
        ...
        mask = features.to(bool).to(torch.float32).to(device)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            valid_avg_loss, valid_avg_acc = compute_acc_and_loss(model, valid_loader, loss_fn, device=device)

            train_avg_loss, train_avg_acc = compute_acc_and_loss(model, train_loader, loss_fn, device=device)
        ...
        eval_metrics = OrderedDict([('loss', train_avg_loss), ('acc', train_avg_acc)])
